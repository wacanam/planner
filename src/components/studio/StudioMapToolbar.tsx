...
export interface StudioMapToolbarProps {
  // Basemap & Layer settings
  mode: BasemapMode;
...
export function StudioMapToolbar({
  mode,
  onSelectMode,
  layers,
  onChangeLayers,
  ...
  return (
    <div className="absolute bottom-5 left-4 sm:bottom-6 sm:left-6 z-30 flex items-center gap-1 sm:gap-1.5 p-1 max-w-[calc(100vw-2rem)] sm:max-w-[calc(100vw-3rem)] overflow-x-auto scrollbar-none touch-pan-x [scrollbar-width:none] [-ms-overflow-style:none] [&::-webkit-scrollbar]:hidden rounded-2xl bg-card/95 backdrop-blur-md border border-border shadow-xl pointer-events-auto">
      ...
      <Button
        ...
        className={`h-9 px-2.5 sm:px-3 rounded-xl font-bold text-xs gap-1.5 shrink-0 whitespace-nowrap transition-all ${
          is3D
      ...
      <Popover open={cameraPopoverOpen} onOpenChange={setCameraPopoverOpen}>
        <PopoverTrigger asChild>
          <Button
            ...
            variant="ghost"
            size="icon"
            className="h-9 w-9 rounded-xl shrink-0 text-muted-foreground hover:text-foreground hover:bg-muted/50"
      ...
      {/* Divider */}
      <div className="h-5 w-px bg-border/80 my-auto mx-0.5 shrink-0" />

      {/* Basemap & Layer Filters Popover */}
      <Popover open={layersOpen} onOpenChange={setLayersOpen}>
        <PopoverTrigger asChild>
          <Button
            ...
            variant="ghost"
            size="icon"
            className="h-9 w-9 rounded-xl text-foreground hover:bg-muted/60 shrink-0 transition-all"
            title={`${mode === 'satellite' ? 'Satellite' : 'Street'} Map Style, Layers & Household Filters`}
          >
