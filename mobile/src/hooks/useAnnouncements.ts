// mobile/src/hooks/useAnnouncements.ts
import {
  collection,
  deleteDoc,
  doc,
...
export function useAnnouncements(congregationId?: string | null) {
  ...
  useEffect(() => {
    ...
    // Subscribe to all announcements collection and filter in-memory for offline robustness
    const unsubscribe = onSnapshot(
      ...
      (err) => {
        console.warn('[useAnnouncements onSnapshot error]', err);
        setError(err);
        setIsLoading(false);
      }
    );
  ...
  const announcements = useMemo(() => {
    ...
    const filtered = allAnnouncements.filter((item) => {
      // System announcements are always visible to everyone
      ...
      // If congregation scope -> visible to all publishers in the congregation
      if (item.scope === 'congregation') return true;

      // If service_group scope
      if (item.scope === 'service_group') {
        if (isGlobalAdmin) return true;
        const cRole = user?.congregationRole || user?.role;
        const isOverseer =
        ...
        // If user is member of that group
        const uGroupId = user?.groupId || (user as any)?.serviceGroupId;
    ...
    // Sort: Pinned first, then by priority (urgent -> important -> normal), then by createdAt desc
  ...
  const createAnnouncement = async (input: CreateAnnouncementInput): Promise<Announcement> => {
    ...
    const newAnnouncement: Announcement = {
      ...
      congregationId:
        input.scope === 'system'
          ? null
          : input.congregationId || congregationId || user.congregationId || null,
      ...
      authorRole: user.congregationRole || user.role || 'SERVICE_OVERSEER',
      createdAt: timestamp,
      updatedAt: timestamp,
  ...
  const togglePin = async (id: string, currentPin: boolean): Promise<void> => {
...
export function useServiceYearAnnouncementSuggestions(
  ...
  const suggestions: ServiceYearSuggestion[] = useMemo(() => {
    const now = new Date();
    const month = now.getMonth(); // 0 = Jan, 7 = Aug, 8 = Sep, etc.
    const day = now.getDate();

    const list: ServiceYearSuggestion[] = [];

    // Check existing announcements in this service year
    ...
    // 1. New / Upcoming Service Year Kickoff (Aug 15 - Oct 15)
    // Month 7 = August (from day 15 onwards), Month 8 = September, Month 9 = October (up to day 15)
    ...
    // Default fallback if outside specific windows (e.g. general periodic pacing)
    if (list.length === 0) {
      list.push({
